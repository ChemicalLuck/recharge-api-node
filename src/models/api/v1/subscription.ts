import { z } from "zod";
import { Money, Property, AnalyticsData } from "~/models/api/common";

/**
 * A Recharge subscription (2021-01).
 *
 * @see https://developer.rechargepayments.com/2021-01/subscriptions/subscriptions_object
 */
export const Subscription = z.looseObject({
  ...
  charge_interval_frequency: z
    .union([z.number(), z.string()])
    .nullable()
    .optional(),
  order_interval_frequency: z
    .union([z.number(), z.string()])
    .nullable()
    .optional(),
...
/** Envelope returned by bulk subscription endpoints. */
...
/** Response returned by `GET /subscriptions/count`. */
export const SubscriptionCountResponse = z.object({ count: z.number() });
export type SubscriptionCountResponse = z.infer<
  typeof SubscriptionCountResponse
>;
...
export const SubscriptionUpdate = z.looseObject({
  ...
  sku: z.string().optional(),
  sku_override: z.boolean().optional(),
  use_shopify_variant_defaults: z.boolean().optional(),
  commit_update: z.boolean().optional(),
  expire_after_specific_number_of_charges: z.number().optional()
});
export type SubscriptionUpdate = z.infer<typeof SubscriptionUpdate>;

/** Body for `POST /subscriptions/{id}/change_next_charge_date`. */
export const SubscriptionChangeNextChargeDate = z.looseObject({
  date: z.string().optional(),
  next_charge_date: z.string().optional()
});
export type SubscriptionChangeNextChargeDate = z.infer<
  typeof SubscriptionChangeNextChargeDate
>;
...
/** Body for `POST /subscriptions/bulk_create`. */
export const SubscriptionBulkCreate = z.looseObject({
  subscriptions: z.array(SubscriptionCreate).optional()
});
export type SubscriptionBulkCreate = z.infer<typeof SubscriptionBulkCreate>;

/** Body for `PUT /subscriptions/bulk_update`. */
export const SubscriptionBulkUpdate = z.looseObject({
  subscriptions: z
    .array(SubscriptionUpdate.extend({ id: z.number().optional() }))
    .optional()
});
export type SubscriptionBulkUpdate = z.infer<typeof SubscriptionBulkUpdate>;

/** Body for `DELETE /subscriptions/bulk_delete`. */
export const SubscriptionBulkDelete = z.looseObject({
  subscriptions: z
    .array(z.looseObject({ id: z.number().optional() }))
    .optional()
});
export type SubscriptionBulkDelete = z.infer<typeof SubscriptionBulkDelete>;
