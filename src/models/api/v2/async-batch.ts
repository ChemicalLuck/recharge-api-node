...
/**
 * A single task within an async batch (2021-11).
 *
 * Each task represents one queued operation (e.g. creating a discount or
 * updating an address) whose body shape depends on the parent batch's
 * `batch_type`. The `body` is left loose because it mirrors whatever resource
 * payload the batch operates on.
 *
 * @see https://developer.rechargepayments.com/2021-11/async_batches/async_batch_object
 */
export const AsyncBatchTask = z.looseObject({
  ...
  status: z.string().nullable().optional(),
  queued_at: z.unknown().nullable().optional(),
  started_at: z.unknown().nullable().optional(),
  completed_at: z.unknown().nullable().optional(),
...
/**
 * A Recharge async batch (2021-11).
 *
